...
namespace ft
{
	...
	class set
	{
		...
		private:
			...
			{
				T&
				operator()(T& _src) const
				{
					return (_src);
				}
			
				const T&
				operator()(T const& _src) const
				{
					return (_src);
				}
			};
		...
		public:
			...
			explicit
			set(const _compare& _comp, const allocator_type& _allocate = allocator_type()):
			...
			ft::set<_key, _compare, _alloc>&
			operator=(const set<_key, _compare, _alloc>& _src)
			{
				_M_rbt = _src._M_rbt;
				return (*this);
			}
			...
			void
			swap(set<_key,_compare,_alloc>& _src)
			{
				_M_rbt.swap(_src._M_rbt);
			}
			...
			friend bool
			operator==(const set<_key1,_compare1,_alloc1>&, const set<_key1,_compare1,_alloc1>&);
